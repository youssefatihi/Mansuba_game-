  ...
  int b=rand()%player1->current_places.size; // to choose randomply one index from the current_places
    ...
    while(tmp==0){
      ...
      if(player1->t[a].type==PRISONNIERE && world_get_sort(world,a)==NO_SORT){
       ...
       //for the escape and the case is not available we search of another cell
        ...
        else if( set_find(other->begining_places,a) || world_get_sort(world,a)==NO_SORT || set_find(player1->begining_places,set.t[move]) ||((set_find(other->begining_places,set.t[move]) ) && world_get_sort(world,set.t[move]!=NO_SORT))) {
          b=(b+1)%player1->current_places.size; // to choose randomply one index from the current_places
          a=player1->current_places.t[b];  
          loop++;
        }
    ...
    if(tmp==2 && set.size!=0){
      //To capture the opponent
       if(other->t[set.t[move]].first_sort_died==NO_SORT){ 
       if(world_get_sort(world,set.t[move])!=NO_SORT ){
              other->t[set.t[move]].type=PRISONNIERE;       
         }
        
        if(player1->t[set.t[move]].type==PRISONNIERE ){
          player1->t[set.t[move]].first_sort_died=player1->t[set.t[move]].sort;
        }
...
int main(int argc,char* argv[]){
  ...
  if(type_relation()==2 || type_relation()==3) horse_number=0;
