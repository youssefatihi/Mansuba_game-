...
#define ANSI_COLOR_VIOLET  "\e[45;30m"
...
struct set possible_moves(struct world_t *w,unsigned int idx,struct player other){
  ...
  for(int i=0;i<4;i++){ // the loop for doing a lot of jumps here the most we can do is 4 jump
    if(!is_empty(set2)){
...
void initialize_player(int dir,struct player* player, struct world_t* world, int tour_number, int elephant_number, int horse_number){
  ...
  while(i<tour_number){ // to add exactly tour_number cells with sort=TOUR
    ...
    if(world_get_sort(world,player->current_places.t[tmp_rand])==PAWN){ // while we didn't find a available place
  ...
  tmp_rand=0;
   while(j<elephant_number){
    tmp_rand=rand()%HEIGHT;
    if(world_get_sort(world,player->current_places.t[tmp_rand])==PAWN){
  ...
  for(int i=0;i<WORLD_SIZE;i++){
    ...
    if(world_get(world,i)==BLACK){
        ...
        case TOUR:
            ...
            break;
        case HORSE:  
             printf(ANSI_COLOR_GRIS" > " ANSI_COLOR_RESET);
            break;
        case ELEPHANT:
...
struct set horse_moves(struct world_t *w,unsigned int idx){
    ...
    for(int i=0; i<tmp.size;i++){
        if(tmp.t[i]!=(int)UINT_MAX &&  world_get_sort(w,tmp.t[i])==NO_SORT) set_add(&set,tmp.t[i]); // to take just the available cells 
    }
...
struct set tour_moves(struct world_t *w, int idx){
    ...
    while(i<=3){
        dir=i;
        tmp=idx;
        while(get_neighbor(tmp,dir)!=UINT_MAX && world_get_sort(w,get_neighbor(tmp,dir))==NO_SORT){ 
            set_add(&set, get_neighbor(tmp,dir));
...
while(i<=3){ // the loop to take the cells with to jumps in same direction 
    dir1=i;
    if(get_neighbor(get_neighbor(idx,dir1),dir1)!=UINT_MAX && world_get_sort(w,get_neighbor(get_neighbor(idx,dir1),dir1))==NO_SORT) 
    ...
    if(i>=0) { // to take cells with two deffrent directions for exemple {SOUTH+EST, SOUTH+WEST}
         dir2=-(i+1);
         dir1=i+1;
        if(get_neighbor(idx,dir2)!=UINT_MAX && world_get_sort(w,get_neighbor(idx,dir2))==NO_SORT)
             set_add(&set,get_neighbor(idx,dir2));
        else if(get_neighbor(idx,dir1)!=UINT_MAX && world_get_sort(w,get_neighbor(idx,dir1))==NO_SORT)
...
void play(struct player *player1,struct world_t *world,struct player other){
  if(player1->current_places.size!=0){  
    ...
    while(set_find(other.begining_places,a)){ // so as not to play with the cells that have arrived
    ...
    if(set.size!=0){ // if possible moves not empty
      ...
      world_set(world,set.t[move],world_get(world,a));
...
int main(int argc,char* argv[]){
  ...
  int other_player=(random_player%2)+1; // random player and other player must be one of them 1 and the other 2
    int elephant_number=1; // the number of ELEPHANT cells for each player
    int horse_number=1; // the number of HORSE cells for each player
    int tour_number=1;  // the number of TOUR cells for each player 
