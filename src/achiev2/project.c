...
void initilize_plateau(struct world_t *world,int seed){
  if(seed==2 || seed==3){
  struct set set={{0},1}; // the cell with the index 0 is the first availeble cell
...
struct set tour_moves(struct world_t *w, int idx){
    ...
    while(i<=3){
        dir=i;
        tmp=idx;
        while(get_neighbor(tmp,dir)!=UINT_MAX && ( world_get_sort(w,get_neighbor(tmp,dir))==NO_CELL || world_get_sort(w,get_neighbor(tmp,dir))==NO_SORT)){ 
            if(world_get_sort(w,get_neighbor(tmp,dir))==NO_SORT) set_add(&set, get_neighbor(tmp,dir));
...
int main(int argc,char* argv[]){
  ...
  printf("%d %d\n",WIDTH,HEIGHT);
  printf("#\n"); 
  //int type_plateau=get_neighbors_seed();
 // printf("-------------------------------%d\n",type_plateau);
